"""Main entry point for the Aye CLI application.

Defines the typer CLI commands for authentication, chat, snapshots, and configuration.
"""
...
def main(
    ...
    """Main entry point for the Aye CLI."""
...
def chat(
    root: Path = typer.Option(
        None, "--root", "-r",
        help="Root folder where source files are located."
    ),
    file_mask: str = typer.Option(
        None, "--include", "-i",
        help="Include patterns for source files. Comma-separated globs."
    ),
    ground_truth: str = typer.Option(
        None, "--ground-truth", "-g", hidden=True,
...
def show_snap(
    ...
    ordinal: str = typer.Argument(..., help="Snapshot ID (e.g., 001)")
) -> None:
...
def restore(
    ordinal: str = typer.Argument(None, help="Snapshot ID to restore (default: latest)"),
    file_name: str = typer.Argument(None, help="Specific file to restore")
) -> None:
...
def keep(
    num: int = typer.Option(10, "--num", "-n", help="Number of recent snapshots to keep")
) -> None:
    ...
    try:
        if action == "list":
            ...
            if not key or value is None:
                raise typer.BadParameter("Key and value are required for set action.")
            commands.set_config_value(key, value)
            cli_ui.print_generic_message(f"Configuration '{key}' set.")
        elif action == "delete":
            if not key:
                raise typer.BadParameter("Key is required for delete action.")
            if commands.delete_config_value(key):
                cli_ui.print_generic_message(f"Configuration '{key}' deleted.")
            else:
                cli_ui.print_generic_message(
                    f"Configuration key '{key}' not found.", is_error=True
                )
        else:
            raise typer.BadParameter(
                f"Invalid action '{action}'. Use: list, get, set, delete"
            )
    except (OSError, ValueError) as e:
