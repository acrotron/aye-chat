"""Command handlers for the Aye REPL.

Provides handlers for built-in commands like cd, model, verbose, debug, and with.
"""
import os
...
def handle_model_command(  # pylint: disable=too-many-branches,too-many-locals
    session: PromptSession, models: list, conf: Any, tokens: list
):
        ...
        try:
            ...
            if 1 <= num <= len(models):
                ...
                if selected_model.get("type") == "offline":
                    download_response = conf.plugin_manager.handle_command(
                    ...
                    if download_response and not download_response.get("success", True):
                        err_msg = download_response.get('error', 'Unknown error')
                        rprint(f"[red]Failed to download model: {err_msg}[/]")
                        return
...
def handle_with_command(  # pylint: disable=too-many-locals,too-many-return-statements
    prompt: str,
    conf: Any,
    console: Console,
    chat_id: int,
    ...
    try:
        ...
        if conf.verbose or len(explicit_source_files) != len(expanded_files):
            file_list = ', '.join(explicit_source_files.keys())
            rprint(f"[cyan]Including {len(explicit_source_files)} file(s): {file_list}[/cyan]")
