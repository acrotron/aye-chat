"""Command implementations for the Aye CLI.

Provides functions for authentication, snapshots, configuration, and project context.
"""
import json
...
from aye.model import auth, config, download_plugins, onnx_manager, snapshot
...
def get_diff_paths(  # pylint: disable=too-many-locals,too-many-branches
    file_name: str,
    snap_id1: Optional[str] = None,
    snap_id2: Optional[str] = None
) -> Tuple[Path, str, bool]:
    """Logic to determine which two files to diff.

    Returns:
        Tuple of (current_file_path, snapshot_reference, is_stash_ref)
        where snapshot_reference is either a file path or a stash reference
        like 'stash@{0}:path/to/file'
    """
...
def initialize_project_context(  # pylint: disable=too-many-branches
    root: Optional[Path],
    file_mask: Optional[str],
    ground_truth_file: Optional[str] = None
) -> Any:
    """Initialize project context by finding root, setting up plugins, and indexing.

    Finds the project root, sets up plugins, and performs initial file scan and index.
    """
    ...
    is_small, _ = _is_small_project(conf.root, conf.file_mask, conf.verbose)
