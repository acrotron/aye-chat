"""LLM invocation module for the Aye CLI.

Handles context preparation, model routing, and response parsing for LLM calls.
"""
import json
import os
...
TRUNCATED_RESPONSE_MESSAGE = (
    ...
    "2. Use the `with` command to focus on specific files: "
    "`with file1.py, file2.py: your request`\n"
...
def _get_rag_context_files(  # pylint: disable=too-many-locals,too-many-branches
    ...
    for file_path_str in unique_files_ranked:
        ...
        try:
            ...
            if current_size + file_size > CONTEXT_HARD_LIMIT:
                if verbose:
                    rprint(
                        f"[yellow]Skipping large file {file_path_str} "
                        f"({file_size / 1024:.1f}KB) to stay within payload limits.[/]"
                    )
                continue
    ...
    # This handles edge cases where accumulated files close to TARGET exceed HARD_LIMIT
    while current_size > CONTEXT_HARD_LIMIT and files_with_sizes:
        ...
        if verbose:
            rprint(
                f"[yellow]Trimmed {removed_path} ({removed_size / 1024:.1f}KB) "
                "to stay within hard limit.[/]"
            )
...
def _determine_source_files(  # pylint: disable=too-many-return-statements
    ...
    is_all_command = (
        stripped_prompt.lower().startswith('/all')
        and (len(stripped_prompt) == 4 or stripped_prompt[4].isspace())
    )
    if is_all_command:
    ...
    if total_size < CONTEXT_HARD_LIMIT:
        if verbose:
            rprint(
                f"[cyan]Project size ({total_size / 1024:.1f}KB) is small; "
                "including all files.[/]"
            )
...
def _print_context_message(
    ...
    use_all_files: bool,
    explicit_source_files: Optional[Dict[str, str]],
    verbose: bool
):
...
def _parse_api_response(resp: Dict[str, Any]) -> Tuple[Dict[str, Any], Optional[int]]:
    ...
    try:
        ...
        if _is_debug():
            print("[DEBUG] Successfully parsed assistant_response JSON")
    except json.JSONDecodeError as e:
        if _is_debug():
            print(
                f"[DEBUG] Failed to parse assistant_response as JSON: {e}. "
        ...
        if "error" in assistant_resp_str.lower():
            ...
            raise RuntimeError(
                f"Server error in chat '{chat_title}': {assistant_resp_str}"
            ) from e
...
def invoke_llm(  # pylint: disable=too-many-arguments,too-many-positional-arguments,too-many-locals
    prompt: str,
    conf: Any,
    console: Console,
    plugin_manager: Any,
    ...
    if hasattr(conf, 'ground_truth') and conf.ground_truth:
        system_prompt = conf.ground_truth
    else:
    ...
    with thinking_spinner(console, messages=spinner_messages, interval=15.0):
        ...
        if _is_debug():
            print(
                f"[DEBUG] Processing chat message with chat_id={chat_id or -1}, "
                f"model={conf.selected_model}"
            )
        ...
        if _is_debug():
            resp_keys = api_resp.keys() if api_resp else 'None'
            print(f"[DEBUG] Chat message processed, response keys: {resp_keys}")
