...
class IndexConfig:
    ...
    def from_params(
        ...
        from .index_manager_utils import MAX_WORKERS  # pylint: disable=import-outside-toplevel
        return cls(
            root_path=root_path,
            file_mask=file_mask,
            verbose=verbose,
...
@dataclass
class IndexingState:  # pylint: disable=too-many-instance-attributes
    """
...
class ProgressTracker:
    ...
    def get_display(self) -> str:  # pylint: disable=too-many-return-statements
        """Get formatted progress display string."""
        acquired = self._lock.acquire(timeout=0.01)  # pylint: disable=consider-using-with
        if not acquired:
            return "indexing..."
        try:
...
class InitializationCoordinator:
    ...
    def initialize(self, blocking: bool = True) -> bool:
        ...
        """
        from aye.model import onnx_manager  # pylint: disable=import-outside-toplevel
        ...
        # Try to acquire lock
        if blocking:
            acquired = self._lock.acquire(timeout=0.1)  # pylint: disable=consider-using-with
        else:
            acquired = self._lock.acquire(blocking=False)  # pylint: disable=consider-using-with
    ...
    def _do_initialize(self) -> bool:
        ...
        from aye.model import vector_db  # pylint: disable=import-outside-toplevel
