...
# =============================================================================
# Tests for focused state components
# =============================================================================

class TestStatusFlags:
    """Tests for the StatusFlags focused dataclass."""

    def test_default_values(self):
        flags = state.StatusFlags()
        assert flags.is_indexing is False
        assert flags.is_refining is False
        assert flags.is_discovering is False
        assert flags.shutdown_requested is False

    def test_is_active_all_false(self):
        flags = state.StatusFlags()
        assert flags.is_active() is False

    def test_is_active_indexing(self):
        flags = state.StatusFlags(is_indexing=True)
        assert flags.is_active() is True

    def test_is_active_refining(self):
        flags = state.StatusFlags(is_refining=True)
        assert flags.is_active() is True

    def test_is_active_discovering(self):
        flags = state.StatusFlags(is_discovering=True)
        assert flags.is_active() is True

    def test_is_active_shutdown_only(self):
        flags = state.StatusFlags(shutdown_requested=True)
        assert flags.is_active() is False


class TestWorkQueues:
    """Tests for the WorkQueues focused dataclass."""

    def test_default_empty(self):
        wq = state.WorkQueues()
        assert wq.files_to_coarse_index == []
        assert wq.files_to_refine == []
        assert wq.target_index == {}

    def test_has_work_empty(self):
        wq = state.WorkQueues()
        assert wq.has_work() is False

    def test_has_work_coarse(self):
        wq = state.WorkQueues(files_to_coarse_index=["a.py"])
        assert wq.has_work() is True

    def test_has_work_refine(self):
        wq = state.WorkQueues(files_to_refine=["b.py"])
        assert wq.has_work() is True

    def test_has_work_both(self):
        wq = state.WorkQueues(files_to_coarse_index=["a.py"], files_to_refine=["b.py"])
        assert wq.has_work() is True

    def test_clear(self):
        wq = state.WorkQueues(
            files_to_coarse_index=["a.py"],
            files_to_refine=["b.py"],
            target_index={"a.py": {"hash": "h"}},
        )
        wq.clear()
        assert wq.files_to_coarse_index == []
        assert wq.files_to_refine == []
...
class TestIndexData:
    """Tests for the IndexData focused dataclass."""

    def test_default_empty(self):
        d = state.IndexData()
        assert d.current_index_on_disk == {}

    def test_set_and_get(self):
        d = state.IndexData()
        d.current_index_on_disk = {"a.py": {"hash": "h"}}
        assert d.current_index_on_disk == {"a.py": {"hash": "h"}}


class TestProgressCounters:
    """Tests for the ProgressCounters focused dataclass."""

    def test_default_values(self):
        pc = state.ProgressCounters()
        assert pc.coarse_total == 0
        assert pc.coarse_processed == 0
        assert pc.refine_total == 0
        assert pc.refine_processed == 0
        assert pc.discovery_total == 0
        assert pc.discovery_processed == 0

    def test_reset_coarse(self):
        pc = state.ProgressCounters(coarse_total=5, coarse_processed=3)
        pc.reset_coarse(10)
        assert pc.coarse_total == 10
        assert pc.coarse_processed == 0

    def test_reset_refine(self):
        pc = state.ProgressCounters(refine_total=5, refine_processed=3)
        pc.reset_refine(7)
        assert pc.refine_total == 7
        assert pc.refine_processed == 0

    def test_reset_discovery(self):
        pc = state.ProgressCounters(discovery_total=5, discovery_processed=3)
        pc.reset_discovery()
        assert pc.discovery_total == 0
        assert pc.discovery_processed == 0
...
# =============================================================================
# Tests for composed IndexingState (backward compatibility)
# =============================================================================
...
def test_indexing_state_composed_status_flags():
    """Test that IndexingState delegates to StatusFlags."""
    st = state.IndexingState()

    # Verify we can access the composed StatusFlags directly
    assert isinstance(st.status, state.StatusFlags)

    # Set via property, read via component
    st.is_indexing = True
    assert st.status.is_indexing is True

    # Set via component, read via property
    st.status.is_refining = True
    assert st.is_refining is True

    # is_active delegates to status
    assert st.is_active() is True
    assert st.status.is_active() is True


def test_indexing_state_composed_work_queues():
    """Test that IndexingState delegates to WorkQueues."""
    st = state.IndexingState()

    assert isinstance(st.work, state.WorkQueues)

    st.files_to_coarse_index = ["a.py"]
    assert st.work.files_to_coarse_index == ["a.py"]

    st.work.files_to_refine = ["b.py"]
    assert st.files_to_refine == ["b.py"]

    st.target_index = {"c.py": {}}
    assert st.work.target_index == {"c.py": {}}

    assert st.has_work() is True
    assert st.work.has_work() is True

    st.clear_work_queues()
    assert st.work.has_work() is False


def test_indexing_state_composed_index_data():
    """Test that IndexingState delegates to IndexData."""
    st = state.IndexingState()

    assert isinstance(st.data, state.IndexData)

    st.current_index_on_disk = {"a.py": {"hash": "h"}}
    assert st.data.current_index_on_disk == {"a.py": {"hash": "h"}}

    st.data.current_index_on_disk["b.py"] = {"hash": "h2"}
    assert st.current_index_on_disk["b.py"] == {"hash": "h2"}


def test_indexing_state_composed_progress_counters():
    """Test that IndexingState delegates to ProgressCounters."""
    st = state.IndexingState()

    assert isinstance(st.counters, state.ProgressCounters)

    st.reset_coarse_progress(10)
    assert st.counters.coarse_total == 10
    assert st.counters.coarse_processed == 0

    st.coarse_processed = 5
    assert st.counters.coarse_processed == 5


def test_indexing_state_generation():
    ...
    st.generation = 99
    assert st.generation == 99


def test_indexing_state_shutdown_flag():
    """Test shutdown_requested flag via property."""
    st = state.IndexingState()

    assert st.shutdown_requested is False
    st.shutdown_requested = True
    assert st.shutdown_requested is True
    assert st.status.shutdown_requested is True
